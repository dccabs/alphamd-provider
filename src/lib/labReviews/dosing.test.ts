...
import { injectionSig, mlPerDose, readDose, weeklyMgLabel } from './dosing.ts'
...
test('a concentration that is not 200mg/mL is refused rather than assumed', () => {
  const dose = readDose({
    name: 'Testosterone cypionate',
...
test('a generated sig is written the way existing ones are', () => {
  assert.equal(
    injectionSig({ weeklyMg: 160, perWeek: 2, route: 'subcutaneously' }),
    'Inject .4mL subcutaneously every 3.5 days.'
  )

  assert.equal(
    injectionSig({ weeklyMg: 200, perWeek: 1, route: 'intramuscularly' }),
    'Inject 1mL intramuscularly once weekly on the same day every week.'
  )

  assert.equal(
    injectionSig({ weeklyMg: 160, perWeek: 3, route: 'subcutaneously' }),
    'Inject .267mL subcutaneously on MWF.'
  )
})
